...
Q5  无法判定 → 不写。标 [Unknown]，记入 MEMORY_ARCHITECTURE.md §10 或该项目 DECISIONS.md，
    等 Human / ChatGPT 裁决。
```
...
| R-10 | Control Tower 究竟是否已建立（两种记载互相矛盾） | **Pending** | 不进事实层 → `agent-lab/MEMORY_ARCHITECTURE.md` §10 U-A | 记载冲突，Buddy 不自行选边 | `[Unknown]` |
| R-11 | 用户偏好表格化输出、倾向保守方案、喜欢 A/B/C 选项 | **Pending** | 不进事实层 → `agent-lab/MEMORY_ARCHITECTURE.md` §10 U-D | 来源在 Buddy 本地 memory（不在 Git）且未经 Human 确认 | `[Unknown]`，待 Human / ChatGPT 裁决 |
